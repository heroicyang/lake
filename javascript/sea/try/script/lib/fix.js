...
define(function(require,exports,module){
            ...
            function ofixed(top, bottom) {
                ...
                if (oscrolltop <= top) {
                    if (oload !== 0) {
                        ...
                        oclone.hide();
                        oload = 0;
                        Athis.find('*').trigger('scrollstatic');       //广播事件 scrollstatic
                    }
                } else if (oscrolltop < bot) {
                        if (oload !== 1) {
                            oclone.show();
                            Athis.css({
                                position: 'fixed',
                            ...
                            oload = 1;
                            Athis.find('*').trigger('scrollfix');       //广播事件 scrollfix
                        }
                    } else if (oload !== 2) {
                            ...
                            oload = 2;
                            Athis.find('*').trigger('scrollrelative');   //广播事件 scrollrelative
                        }
            }
            /*注册事件*/
    ...
    $.fn.fix.defaults = {
        ...
        bottom:0            //当元素距离页面底部多少像素时开始static
    }
})