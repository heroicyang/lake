...
window.onload = function () {
    ...
    var width = 800,         //宽
        ...
        X = (width - leftgutter) / labels.length,              //每列宽
        max = Math.max.apply(Math, data),                      //数据最高值
        Y = (height - bottomgutter - topgutter) / max;         //根据最高值确定每行高
    r.drawGrid(leftgutter + X * .5 + .5, topgutter + .5, width - leftgutter - X, height - topgutter - bottomgutter, 10, 10, "#000");
    var path = r.path().attr({stroke: color, "stroke-width": 4, "stroke-linejoin": "round"}),    //折线段
        bgp = r.path().attr({stroke: "none", opacity: .3, fill: color}),                         //折线区域
        label = r.set(),                                                                         
        lx = 0, ly = 0,
    ...
    var p, bgpp;                 //p为折线段数组 //bgpp为折线区域数组
    for (var i = 0, ii = labels.length; i < ii; i++) {
        var y = Math.round(height - bottomgutter - Y * data[i]),                  //获取y值                                
            x = Math.round(leftgutter + X * (i + .5)),                            //获取x值                      
